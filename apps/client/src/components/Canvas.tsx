...
const Canvas = () => {
  ...
  const {
    strokes,
    actions: { drawStroke, clearCanvas },
    undoStroke,
    isDrawer,
  } = useGameStore();
  ...
  return (
    <main className="flex flex-col relative items-center justify-center bg-gray-300 h-full">
      <div
        className={`relative rounded-lg overflow-hidden ${isDrawer ? "cursor-pointer" : "cursor-default"}`}
      >
        <canvas
      ...
      {isDrawer && (
        <div
          className="py-2 flex gap-4 items-center bg-gray-800 text-white
        ...
        >
          ...
          <button
            onClick={undoStroke}
            className="px-3 py-1 hover:bg-gray-700 rounded-md"
          >
            Undo
          </button>
          ...
          <button
            onClick={handleClear}
            className="px-3 py-1 hover:red-700 text-red-400 rounded-md"
          >
            Clear
          </button>
        </div>
