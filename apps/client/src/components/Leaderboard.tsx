    ...
    return (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90 z-50 text-white">
            ...
            <div className="bg-gray-800 rounded-xl p-8 shadow-2xl w-full max-w-lg">
                <h3 className="text-2xl font-bold mb-6 text-center border-b border-gray-700 pb-4">Leaderboard</h3>
                
                <ul className="space-y-4">
                    {sortedUsers.map((user, index) => {
                        let rowClass = "flex justify-between items-center p-3 rounded-lg text-lg";
                        let rankIcon = null;

                        if (index === 0) {
                            rowClass += " bg-yellow-500/20 text-yellow-300 font-bold border border-yellow-500/50";
                            rankIcon = "👑";
                        } else if (index === 1) {
                        ...
                        return (
                            <li key={user.id} className={`${rowClass} ${isMe ? "ring-2 ring-blue-500" : ""}`}>
                                <div className="flex items-center gap-4">
                                    <span className="w-8 text-center">{rankIcon}</span>
                                    <span>
                                        {user.name} 
                                        {isMe && <span className="text-sm ml-2 text-blue-400">(You)</span>}
                                    </span>
                                </div>
                                <span className="font-mono font-bold text-xl">{user.score}</span>
                            </li>
                        );
                    })}
                </ul>
